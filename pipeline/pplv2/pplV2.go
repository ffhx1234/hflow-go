...
func (p *Pipeline) RegisterNode(name string, node Node) *Pipeline {
	if name == "" || node == nil {
		log.Panicln("node name or node is nil")
	}
	nodeLink := &NodeLink{
		node: node,
		name: name,
	}
	p.nodes[name] = nodeLink
	return p
}

func (p *Pipeline) Connect(parent string, child []string) *Pipeline {
	if parent == "" || len(child) == 0 {
		log.Panicln("parent or child is nil")
	}
	p.lnMux.Lock()
	defer p.lnMux.Unlock()
	p.linkNodes[parent] = child
	return p
}
